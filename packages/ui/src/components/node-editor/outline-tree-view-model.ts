...
import { canInsertOutline, type OutlineItemCapabilities } from "./outline-capabilities.js";
...
export type OutlineItemViewModel<Presentation = unknown> = Readonly<{
  /** The host can choose the insertion position at a collapsed row's text end. */
  endInsertion?: "after" | "child";
  accessibilityLabel: string;
  children?: readonly OutlineItemViewModel<Presentation>[];
  content: OutlineContent;
  capabilities?: OutlineItemCapabilities;
  editable?: boolean;
  href?: string;
  /** An uncommitted empty insertion uses the existing editor without becoming persisted content. */
  placeholder?: boolean;
...
export type OutlineMoveDestination = Readonly<{
  /** Insertion position after the moving siblings have been removed. */
  index: number;
...
export type OutlineMove =
  | Readonly<{
      kind: "absolute";
      /** Gap in the displayed list before removing the dragged rows. */
      index: number;
      sourceKeys: readonly string[];
      targetParentKey: string | null;
    }>
  | Readonly<{
      kind: "relative";
      sourceKeys: readonly string[];
      operation: "indent" | "outdent" | "reorder-up" | "reorder-down";
    }>;

/** The host maps appearances whose opaque keys change after a move. */
export type OutlineMoveResult = Readonly<{ keyMap: ReadonlyMap<string, string>; revealKeys?: readonly string[] }>;
...
export function flattenOutline<Presentation>(
  ...
  const visit = (siblings: readonly OutlineItemViewModel<Presentation>[], parentKey: string | null, depth: number) => {
    siblings.forEach((item, indexInParent) => {
      ...
      if (expanded && item.children !== undefined && !embedded?.has(item.key)) {
...
function siblingSelection(
  ...
  if (
    ...
    selected.length === 0 ||
    selected.some((row) => row.item.capabilities?.move === false) ||
...
export function computeIndent(
  ...
  if (target === undefined || !canInsertOutline(target.item, "child")) {
    return null;
  }
  return {
...
export function computeOutdent(
  ...
  if (parent === undefined || !canInsertOutline(parent.item, "after")) {
    return null;
  }
  return {
...
export function computeEditMergeTarget(
  ...
  if (
    !source ||
    source.hasChildren ||
    source.item.capabilities?.remove === false ||
...
export function computeEditInsertion(
  ...
  if (row === undefined || !canInsertOutline(row.item, placement)) {
    return null;
  }
...
/** Resolve relative keyboard intent only against the snapshot that will apply it. */
export function resolveOutlineMove(
  rows: readonly OutlineRowViewModel[],
  move: OutlineMove,
): OutlineMoveDestination | null {
  if (move.kind === "absolute") {
    const selected = new Set(move.sourceKeys);
    const preceding = rows.filter(
      (row) => selected.has(row.key) && row.parentKey === move.targetParentKey && row.indexInParent < move.index,
    ).length;
    return {
      index: Math.max(0, move.index - preceding),
      sourceKeys: move.sourceKeys,
      targetParentKey: move.targetParentKey,
    };
  }
  return move.operation === "indent"
    ? computeIndent(rows, move.sourceKeys)
    : move.operation === "outdent"
      ? computeOutdent(rows, move.sourceKeys)
      : computeReorder(rows, move.sourceKeys, move.operation === "reorder-up" ? -1 : 1);
}
