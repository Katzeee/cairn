...
describe("outline demo presenter", () => {
  it("supplies the reason for readonly names without imposing it on editable nodes", () => {
    expect(presentedItem(initialGraph, "projects/cairn/status-field")).toMatchObject({
      editable: false,
  ...
  it("presents Original and Reference children from one target Node", () => {
    const referencePath = "projects/cairn/roadmap/local-first-reference";
  ...
  it("updates shared Node content once for every presented occurrence", () => {
    ...
    expect(presented.modelPathsByKey.get(reference?.key ?? "")).toBe("projects/cairn/roadmap/local-first-reference");
  });
  ...
  it("edits each Field Reference through its target identity", () => {
    for (const [path, original, id] of [
      [
        "projects/cairn/status-field/in-progress",
        "archive/value-library/status-in-progress-original",
        "status-in-progress",
      ],
      ["projects/cairn/owner-field/kei-owner", "kei", "kei"],
      ["projects/cairn/owner-field/team-owner", "archive/value-library/cairn-team-original", "cairn-team"],
    ] as const) {
  ...
  it("retains targets that remain reachable through inline references when a Field is retargeted", () => {
    ...
    const graph = retargetGraphOccurrence(withoutOriginal, "kei-owner", "cairn-team", "reference");
    expect(graph.nodes.kei?.value.content).toEqual(initialGraph.nodes.kei?.value.content);
    expect(graph.occurrences["kei-owner"]?.nodeId).toBe("cairn-team");
  });
  ...
  it("derives option labels and datatype from their owning Nodes", () => {
    ...
    const valueKey = [...presented.modelPathsByKey].find(
      ([, path]) => path === "projects/cairn/status-field/in-progress",
    )?.[0];
  ...
  it("presents a Field from the Node alone, wherever the Field is moved", () => {
    const fieldPath = "projects/cairn/review-date-field";
    ...
    for (const targetPath of ["projects/cairn/owner-field/team-owner", "projects/cairn/owner-field"]) {
