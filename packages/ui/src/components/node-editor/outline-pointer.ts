...
export function useOutlinePointer(options: PointerOptions) {
  ...
  return (row: OutlineRowViewModel, event: MouseEvent) => {
    ...
    select(emptyOutlineSelection);
    cursor(row.key);
    if (row.item.activation === "navigate") {
      event.preventDefault();
      edit.commitAndExit();
      optionsRef.current.onActivate(row);
      return;
    }
    if (row.item.editable === false) {
      edit.navigateTo({ key: row.key, caret: 0, editing: false });
      return;
    }
    ...
    if (event.detail >= 2) {
      event.preventDefault();
      const { from, to } = nodeTextPointerSelection(event.currentTarget, row.item.content, event);
    ...
    event.preventDefault();
    const { from: caret } = nodeTextPointerSelection(event.currentTarget, row.item.content, event);
