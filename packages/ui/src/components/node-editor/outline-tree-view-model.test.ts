...
describe("structure intents", () => {
  ...
  it("resolves a pointer gap against the remaining siblings without counting descendants", () => {
    const items = [
      item("a", "A", [item("child", "Child")]),
      item("b", "B"),
      item("c", "C"),
      item("d", "D"),
      item("e", "E"),
    ];
    const rows = flattenOutline(items, new Set(["a"]));
    for (const [sources, gap, expected] of [
      [["a"], 3, ["b", "c", "a", "d", "e"]],
      [["a", "b"], 4, ["c", "d", "a", "b", "e"]],
      [["d"], 0, ["d", "a", "b", "c", "e"]],
    ] as const) {
      const move = resolveOutlineMove(rows, {
        kind: "absolute",
        index: gap,
        sourceKeys: sources,
        targetParentKey: null,
      })!;
      const remaining = items.map((item) => item.key).filter((key) => !(sources as readonly string[]).includes(key));
      remaining.splice(move.index, 0, ...move.sourceKeys);
      expect(remaining).toEqual(expected);
    }
    expect(resolveOutlineMove(rows, { kind: "relative", operation: "reorder-down", sourceKeys: ["a"] })?.index).toBe(1);
  });
