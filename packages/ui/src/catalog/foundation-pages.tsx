  ...
  return (
    <div className="flex w-40 flex-col flex-nowrap gap-2">
      <span
        ...
        style={{ background: `var(--cairn-color-${role})` }}
      />
  ...
  return (
    <>
      ...
      <Specimen
        ...
        description="Every Cairn surface — desktop window, preview frame, or phone — resolves the same classes from its own width. Components adapt with container queries, not device detection."
        ...
        {spaceSteps.map(([name, value]) => (
          <div className="flex flex-col flex-nowrap items-center gap-2" key={name}>
            <span
              ...
              style={{ height: `calc(var(--cairn-spacing) * ${value / 4})` }}
            />
        ...
        {radii.map(([name, value]) => (
          <div className="flex flex-col flex-nowrap items-center gap-2" key={name}>
            <span
              ...
              style={{ borderRadius: `min(var(--cairn-radius-${name}), 32px)` }}
            />
      ...
      >
        {["xs", "sm", "md", "lg"].map((step) => (
          <div className="flex flex-col flex-nowrap items-center gap-2" key={step}>
            <span className="size-20 rounded-md bg-card" style={{ boxShadow: `var(--cairn-shadow-${step})` }} />
