...
export { flattenOutline, resolveOutlineMove } from "./outline-tree-view-model.js";
export type { OutlineMoveDestination } from "./outline-tree-view-model.js";
...
export function OutlineTree<Presentation, Action>(props: OutlineTreeProperties<Presentation, Action>) {
  return (
    <NodeEditor>
      <OutlineTreeRegion {...props} />
    </NodeEditor>
  );
}

function OutlineTreeRegion<Presentation, Action>({
  childrenViews,
  commands,
  inlineExtensions = [],
  editing,
  ...
  const rows = useMemo(
    () => flattenOutline(items, expandedKeys, childrenViews && new Set(childrenViews.keys())),
    [expandedKeys, items, childrenViews],
  );
  ...
  return (
    <>
      <div
        ...
        onCopyCapture={(event) => {
          if ((event.target as Element).closest('[role="tree"]') === event.currentTarget) {
            interaction.clipboard.onCopyCapture(event);
          }
        }}
        onCutCapture={(event) => {
          if ((event.target as Element).closest('[role="tree"]') === event.currentTarget) {
            interaction.clipboard.onCutCapture(event);
          }
        }}
        onPasteCapture={(event) => {
          if ((event.target as Element).closest('[role="tree"]') === event.currentTarget) {
            interaction.clipboard.onPasteCapture(event);
          }
        }}
        ref={containerRef}
        role="tree"
      ...
      >
        ...
        {!selectionToolbar || nodeSelection.keys.size === 0 ? null : (
          <OutlineSelectionToolbar
            ...
            onDelete={
              onDeleteSelection === undefined || !interaction.canDeleteSelected ? undefined : interaction.deleteSelected
            }
            onMove={onMove === undefined || !interaction.canMoveSelected ? undefined : interaction.moveSelected}
          />
        )}
      ...
      {editing?.onCreateRoot !== undefined && items.at(-1)?.placeholder !== true ? (
