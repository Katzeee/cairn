...
import { canInsertOutline } from "./outline-capabilities.js";
import {
...
export type OutlineEditIntentContext = Readonly<{
  ...
  boundary?: (direction: -1 | 1) => boolean;
  ...
  switch (command.type) {
    ...
    case "navigate": {
      ...
      if (context.editing === undefined || position === null) {
        return context.boundary?.(command.direction) ?? false;
      }
      context.commit(key, command.content);
...
export function insertFromEditor(
  ...
  const placement =
    ...
    (atStart
      ? "before"
      : from === to && to === contentLength(content) && row?.item.endInsertion
        ? row.item.endInsertion
  ...
  const create = forced !== undefined || atStart || (from === to && to === contentLength(content));
  const handler = create
    ? placement === "before"
      ? editing.onCreateBefore
      : placement === "child"
        ? editing.onCreateChild
        : editing.onCreateAfter
    : editing.onSplit;
  if (handler === undefined) {
    return true;
  }
  context.commit(key, content);
  let position: OutlineEditPosition | void;
  if (forced !== undefined || atStart || (from === to && to === contentLength(content))) {
    if (placement === "before") {
      position = editing.onCreateBefore?.(key);
    } else if (placement === "child") {
      position = editing.onCreateChild?.(key);
    } else {
      position = editing.onCreateAfter?.(key);
    }
  } else {
    const split = splitContent(content, from, to);
    position = editing.onSplit?.(key, split.before, split.after, placement === "child" ? "child" : "after");
  }
  if (position) {
    context.activate(position);
  } else {
...
function backspaceFromEditor(context: OutlineEditIntentContext, key: string, content: OutlineContent): boolean {
  ...
  const parent =
    ...
    source.item.activation !== "object"
      ...
      : undefined;
  const previous =
    computeEditNavigation(rows, key, -1, "end") ??
    (parent ? { key: parent.key, caret: contentLength(parent.content) } : null);
  if (previous === null) {
    return false;
  }
  if (contentLength(content) === 0) {
    if (rows.find((row) => row.key === key)?.item.capabilities?.remove === false) {
      return true;
    }
    if (editing.onDeleteEmpty === undefined) {
      return true;
    }
    context.commit(key, content);
  ...
  const merge =
    computeEditMergeTarget(rows, key, content) ??
    (parent
      ? { key: parent.key, caret: contentLength(parent.content), content: mergeContent(parent.content, content) }
      : null);
  const target = merge === null ? undefined : rows.find((row) => row.key === merge.key);
  if (merge === null || (target === undefined && parent === undefined) || target?.item.editable === false) {
    return false;
  }
  context.commit(key, content);
