...
async function verifyOutlinePresentation(page) {
  ...
  const cairnBullet = rowByText("Cairn").locator('[data-ui="outline-bullet"]');
  await cairnBullet.hover();
  assert.equal(
    await cairnBullet.evaluate((element) => getComputedStyle(element).cursor),
    "pointer",
    "a clickable bullet must use the pointer cursor before a drag starts",
  );
  assert.equal(
    await cairnBullet.evaluate((element) => getComputedStyle(element).backgroundColor),
    "rgba(0, 0, 0, 0)",
  ...
  const ownerField = rowByPath("projects/cairn/owner-field");
  ...
  assert.equal(
    await childrenOfPath("projects/cairn/owner-field").count(),
    2,
    "the Owner Field must present Kei and Cairn team as its two Value occurrences",
  );
  assert.equal(
    await rowByPath("projects/cairn/owner-field")
  ...
  const referenceRow = rowByPath("projects/cairn/roadmap/local-first-reference");
  assert.equal(
    await referenceRow.getAttribute("data-item-key"),
    itemKey("projects/cairn/roadmap/local-first-reference"),
  );
  assert.equal(
  ...
  const referenceChild = rowByPath("projects/cairn/roadmap/local-first-reference/local-first-summary");
  assert.equal(
  ...
  const firstOwnerBox = await rowByPath("projects/cairn/owner-field/kei-owner").boundingBox();
  const secondOwnerBox = await rowByPath("projects/cairn/owner-field/team-owner").boundingBox();
  ...
  assert.equal(
    await tree
      .locator(`[data-item-key=${JSON.stringify(itemKey("projects/cairn/status-field/in-progress"))}]`)
  ...
  const cairnEditingRow = rowByPath("projects/cairn");
  const cairnSupertagBeforeEditing = await cairnEditingRow.locator('[data-ui="outline-row-badge"]').boundingBox();
  await cairnEditingRow.locator('[data-ui="outline-row-text"]').click();
  await editor.waitFor({ state: "visible" });
  const cairnEditingBackground = await cairnEditingRow.evaluate((element) => getComputedStyle(element).backgroundColor);
  assert.equal(
    cairnEditingBackground,
    "rgba(0, 0, 0, 0)",
  ...
  assert.equal(
    await cairnEditingRow.locator('[data-ui="outline-row-badge"]').count(),
    0,
  ...
  assert.ok(
    supertagBox !== null &&
      cairnSupertagBeforeEditing !== null &&
      Math.abs(cairnSupertagBeforeEditing.x - supertagBox.x) <= 1,
    "leaving source editing restores the Supertag's inline position",
  );

  const pendingMilestone = rowByPath("projects/cairn/roadmap/outline-m2");
  ...
  const statusValue = rowByPath("projects/cairn/status-field/in-progress");
  ...
  await rowByPath("projects/cairn/review-date-field").click();
  ...
  const statusValues = childrenOfPath("projects/cairn/status-field");
  ...
  const cairnTeam = rowByPath("projects/cairn/owner-field/team-owner");
  await cairnTeam.getByRole("button", { name: "Expand Cairn team" }).click();
  const cairnTeamPlaceholder = tree.locator(
    `[data-ui="outline-empty-child-placeholder"][data-parent-key=${JSON.stringify(itemKey("projects/cairn/owner-field/team-owner"))}]`,
  );
  await cairnTeamPlaceholder.waitFor({ state: "visible" });
  const [cairnTeamBox, cairnTeamPlaceholderBox] = await Promise.all([
    cairnTeam.boundingBox(),
    cairnTeamPlaceholder.boundingBox(),
  ]);
  assert.ok(
    cairnTeamBox !== null && cairnTeamPlaceholderBox !== null && cairnTeamPlaceholderBox.x >= cairnTeamBox.x,
    "an empty-child placeholder under a Field Value must remain in the value column",
  );
  const cairnTeamBulletBox = await cairnTeam.locator('[data-ui="outline-bullet"]').boundingBox();
  const cairnTeamChildBulletBox = await cairnTeamPlaceholder.locator('[data-ui="outline-bullet-mark"]').boundingBox();
  assert.ok(
    cairnTeamBulletBox !== null && cairnTeamChildBulletBox !== null && cairnTeamChildBulletBox.x > cairnTeamBulletBox.x,
    "an empty-child placeholder must indent locally from its Field Value parent",
  );
  assert.equal(
    await childrenOfPath("projects/cairn/owner-field/team-owner").count(),
    0,
    "expanding an empty Node must not materialize a model Node",
  );
  assert.equal(await cairnTeamPlaceholder.locator('[data-ui="outline-placeholder-bullet"]').count(), 1);
  assert.doesNotMatch(
    (await cairnTeamPlaceholder.textContent()) ?? "",
    /Type \/ for commands/u,
  ...
  const emptyChildPlaceholder = tree.locator(
    `[data-ui="outline-empty-child-placeholder"][data-parent-key=${JSON.stringify(itemKey("projects/cairn/roadmap/empty-container"))}]`,
  );
  ...
  const [emptyContainerBulletBox, emptyPlaceholderBulletBox] = await Promise.all([
    rowByPath("projects/cairn/roadmap/empty-container").locator('[data-ui="outline-bullet"]').boundingBox(),
  ...
  const emptyChild = childrenOfPath("projects/cairn/roadmap/empty-container");
  ...
  await editor.waitFor({ state: "visible" });
  await editor.press("Enter");
  const emptyChildren = childrenOfPath("projects/cairn/roadmap/empty-container");
  ...
  await page.locator('[data-ui="outline-row"][aria-level="1"]', { hasText: "Cairn" }).first().waitFor({
    state: "visible",
  });
...
async function verifyOutlineEditing(page) {
  ...
  await rowByPath("projects/cairn/home-lab").locator('[data-ui="outline-editor"]').waitFor();
  assert.equal(
  ...
  const createdField = page
    .locator(
      '[data-ui="outline-node"][data-children-layout="beside"] > .cairn-outline-layout > [data-ui="outline-row"]',
      { hasText: "Notes" },
    )
    .first();
...
async function verifyOutlineDragging(page) {
  ...
  const itemKey = (modelPath) => `outline-item:${encodeURIComponent(modelPath)}`;
  const rowByPath = (modelPath) => tree.locator(`[data-item-key=${JSON.stringify(itemKey(modelPath))}]`);
  const cairnRow = rowByPath("projects/cairn");
  await cairnRow.getByRole("button", { name: "Collapse Cairn" }).click();
  const handle = rowByText("Home lab notes").locator('[data-ui="outline-bullet"]');
  await handle.scrollIntoViewIfNeeded();
  const handleBox = await handle.boundingBox();
  const cairnBox = await cairnRow.boundingBox();
  assert.ok(handleBox !== null && cairnBox !== null, "drag geometry must be measurable");
  await page.mouse.move(handleBox.x + handleBox.width / 2, handleBox.y + handleBox.height / 2);
  await page.mouse.down();
  await page.mouse.move(handleBox.x + handleBox.width / 2, cairnBox.y + 3);
  ...
  assert.equal(await moved.getAttribute("aria-posinset"), "1", "the dragged row must land before Cairn");

  // Dropping a subtree into its own descendant must be rejected.
  await cairnRow.getByRole("button", { name: "Expand Cairn" }).click();
...
designSystemTest("outline Nodes move independently of Field presentation columns", async (page) => {
  ...
  const reviewField = rowByPath("projects/cairn/review-date-field");
  const statusField = rowByPath("projects/cairn/status-field");
  ...
  await reset();
  const roadmap = rowByPath("projects/cairn/roadmap");
  const ownerValue = rowByPath("projects/cairn/owner-field/team-owner");
  ...
  const movedRoadmap = rowByPath("projects/cairn/owner-field/roadmap");
  assert.equal(
    await movedRoadmap.getAttribute("data-parent-key"),
    itemKey("projects/cairn/owner-field"),
  ...
  const [movedRoadmapBox, keiBox] = await Promise.all([
    ...
    rowByPath("projects/cairn/owner-field/kei-owner").boundingBox(),
  ]);
  assert.ok(
  ...
  await reset();
  const fieldInField = rowByPath("projects/cairn/review-date-field");
  const firstOwnerValue = rowByPath("projects/cairn/owner-field/kei-owner");
  assert.ok(
  ...
  const directFieldPath = "projects/cairn/owner-field/review-date-field";
  const directField = rowByPath(directFieldPath);
  const directValue = rowByPath(`${directFieldPath}/review-date-value`);
  const ownerField = rowByPath("projects/cairn/owner-field");
  assert.equal(await directField.getAttribute("data-parent-key"), itemKey("projects/cairn/owner-field"));
  ...
  await reset();
  const nestedReviewField = rowByPath("projects/cairn/review-date-field");
  const nestedOwnerValue = rowByPath("projects/cairn/owner-field/team-owner");
  ...
  const nestedFieldPath = "projects/cairn/owner-field/team-owner/review-date-field";
  ...
  assert.equal(
    ...
    itemKey("projects/cairn/owner-field/team-owner"),
  ...
  await reset();
  const detachableValue = rowByPath("projects/cairn/owner-field/team-owner");
  const detachTarget = rowByPath("projects/cairn/review-date-field");
  assert.ok(
    (await drag(detachableValue, detachTarget, { edge: "before" })) !== null,
    "the ordinary parent position must expose a drop target",
  );
  const detachedValue = rowByPath("projects/cairn/team-owner");
  assert.equal(
  ...
  const [detachedValueBox, siblingFieldBox] = await Promise.all([
    ...
    rowByPath("projects/cairn/review-date-field").boundingBox(),
  ]);
  assert.ok(
