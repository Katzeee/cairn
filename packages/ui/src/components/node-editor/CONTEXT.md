...
Node editing components operate on opaque appearances and host-supplied content bindings. A panel coordinates active editing and focus across its heading and embedded collections without owning persistent Node data. The host supplies domain-specific presentation and actions.

Outline preserves tree interaction semantics inside any collection layout. A table supplies cell geometry and embeds Outline regions; an embedded child view owns its visible targets, so the surrounding Outline does not route keyboard or clipboard operations through hidden rows. Content bindings and structural capabilities remain distinct: a template default, a definition name, and an instance value can use the same source editor while submitting different host intents.
...
_Avoid_: domain editor, recursive renderer
