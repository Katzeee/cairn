...
import { contentLength } from "../components/node-editor/outline-content.js";
...
export function OutlinePage() {
  const initialExpanded = useMemo<ReadonlySet<string>>(() => {
    const expandedModelPaths = new Set([
      "projects",
      "projects/cairn",
      "projects/cairn/roadmap",
      "projects/cairn/roadmap/local-first-reference",
      "field-definitions",
      "inbox",
    ]);
    return new Set(
  ...
  const applyMove = (intent: OutlineMove) => {
    const move = resolveOutlineMove(
      flattenOutline(presentedOutline.items, new Set(presentedOutline.modelPathsByKey.keys())),
      intent,
    );
    if (!move) {
      return null;
